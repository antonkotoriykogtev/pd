<template>
  ...
  <div>
    ...
    <div class="container is-max-desktop searchbar" style="margin-top:50px" >
      <div class="tile is-parent">
        <article class="tile is-child">

          <section v-show="EventDetailNoAuth">
            <div class="container is-max-desktop searchbar" >
              <article class="media" v-for="i in media" :key="i">
                <div class="media-content">
                  ...
                  <nav class="level is-mobile">
                    ...
                    <b-skeleton active style="margin-right: 100px"></b-skeleton>
            ...
            <figure class="image ">
              <img
                :src="event_edit.picture_link"
                style="border-radius:10px;"
              >
            </figure>

            <h1 class="title is-1 MarginTextDetail" style="margin-bottom: 10px; display: inline-block;">{{ event_edit.name }}</h1>



              <p class="template_info_type" style="margin-bottom: 10px;" v-if="event_edit.is_online">
                <b-icon icon="signal"></b-icon>
                Онлайн мероприятие</p>
              <p class="template_info_type" style="margin-bottom: 10px;"  v-if="!event_edit.is_online">
                <b-icon icon="building"></b-icon>
                Оффлайн мероприятие</p>
              <p class="template_info_date">{{ new Date(event_edit.start_date).toLocaleString('ru', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
                hour: 'numeric',
                minute: 'numeric',
                }) }} - {{ new Date(event_edit.end_date).toLocaleString('ru', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
                hour: 'numeric',
              ...
              <div v-if="!event_edit.is_online">
                <p class="title is-3" > Адрес:</p>

                <p class="subtitle is-5">{{ event_edit.address }}</p>
              </div>
              <div class="tile is-ancestor" style="margin-top:0.8rem">
                        ...
                        <article  class="media">
                          <div class="media-left">
                            <figure class="image is-64x64  " >
                              <img class="is-rounded"  :src="profile.picture_link" alt="Image">
                            </figure>
                          </div>
                          <div class="media-content">
                            <div class="content"  >
            ...
            <div class="template_content_description" v-html="event_edit.description"></div>

        </article>
      </div>
    </div>
    <main class="creator">
      <h1 class="title is-4">Редактирование мероприятия</h1>
      <form v-on:submit.prevent="EditEvent">
        <b-field label="Название" >
          <b-input required v-model="event_edit.name" ></b-input>
        </b-field>
        <b-field  v-on:change="handleFileUpload()" class="file is-info " :class="{'has-name': !!file}">
          <b-upload required v-model="file" class="file-label">
            <span class="file-cta">
        ...
        <!--<p class="creator_title">Фото</p>
        &lt;!&ndash;<label>Files
          <input type="file" id="file" ref="file" v-on:change="handleFileUpload()"/>
        </label>&ndash;&gt;-->
        <b-field label="Введите место проведения" >
          <b-input  v-model="event_edit.address" ></b-input>
        </b-field>
        <b-field>
          <b-checkbox v-model="event_edit.is_online"
                      type="is-info"
          >Онлайн</b-checkbox>
        </b-field>

        <p class="creator_title">Дата проведения:</p>
        <!--<input name="eventStart" type="datetime-local" v-model="event.start_date">-->
        <section>
          <b-field required label="Начало">
            <b-datetimepicker
        ...
        <b-field label="Конец">
          <b-datetimepicker
            ...
            :timepicker="{ enableSeconds, hourFormat }"
            horizontal-time-picker
            v-model="event_edit.end_date"
          >
          </b-datetimepicker>
        </b-field>

        <!--<p>Конец: <input name="eventFinish" type="datetime-local" v-model="event.end_date"></p>-->
        <b-field label="Введите стоимость" >
          <b-input min="0" v-model="event_edit.price" placeholder="Оставьте поле пустым для бесплатного мероприятия"></b-input>
        </b-field>
        <b-field label="Лимит количества посетителей">
          <b-slider required type="is-info" indicator :tooltip="false" :max="500"  v-model="event_edit.max_participants_quantity"></b-slider>
        </b-field>
        ...
        <p class="creator_title">Описание:</p>

        <vue-editor  v-model="event_edit.description"></vue-editor>


        <div class=" has-text-centered">
          <button class=" button is-info" type="submit" v-on:click="EditEvent">Опубликовать</button>
        </div>
    ...
    <div class="overlay">
      <div class="login">
        <h1>Вход</h1>
        <button type="button" name="button" class='close-btn is-info' v-on:click="closeLogin"><div style='transform:rotate(45deg);'>+</div></button>
        <form class="login_form"  action="" method="">
          ...
          <button
            ...
            class="button is-info overlay-btn"
            type="submit"
            name="button">Войти</button>
        </form>
      </div>
      <div class="checkin">
        <h1>Регистрация</h1>
        <button type="button" name="button" class='close-btn is-info' v-on:click="closeCheck"><div style='transform:rotate(45deg);'>+</div></button>
        <form class="checkin_form" action="" method="POST">
          <input type="" name="" value="" placeholder="Имя">
          <input type="" name="" value="" placeholder="Email">
        </form>
        <button type="button" class="button is-info overlay-btn" name="button">Зарегистрироваться</button>
      </div>
  ...
  export default {
    ...
    data () {
      return {
        ...
        event_edit:{
          ...
          description: "",
          price: 0,
          max_participants_quantity: null,
          address: "",
          geopoint_lat: null,
          geopoint_long: null,
          ics_file_link: "",
    ...
    methods: {
      EditEvent() {
        ...
        let StartDateNot = this.event.start_date;
        let StartDateString=StartDateNot.toString()
        let StartTime=StartDateString.slice(16,24)
        let start_date_fin =
          StartDateNot.getFullYear() +
          "-" +
          ("0" + (StartDateNot.getMonth() + 1)).slice(-2) +
          "-" +
          ("0" + StartDateNot.getDate()).slice(-2) + "T" + StartTime + "Z";
        console.log(start_date_fin)
        let EndDateNot = this.event.end_date;
        let EndDateString=EndDateNot.toString()
        let EndTime=EndDateString.slice(16,24)
        let end_date_fin =
          EndDateNot.getFullYear() +
          "-" +
          ("0" + (EndDateNot.getMonth() + 1)).slice(-2) +
          "-" +
          ("0" + EndDateNot.getDate()).slice(-2) + "T" + EndTime + "Z";

        axios.defaults.xsrfCookieName = 'csrftoken'
        axios.defaults.xsrfHeaderName = 'X-CSRFToken'
        let TagsArray = JSON.stringify(this.tags)
        console.log(TagsArray)
        this.tags = "5"
        const DataCreationForm = new FormData();
        DataCreationForm.append('picture_link', this.file, this.file.name);
        DataCreationForm.append('author', this.user_id);
        DataCreationForm.append('name', this.event.name);
        DataCreationForm.append('start_date', start_date_fin);
        DataCreationForm.append('end_date', end_date_fin);
        DataCreationForm.append('description', this.event.description);
        DataCreationForm.append('price', this.event.price);
        DataCreationForm.append('is_online', this.event.is_online);
        DataCreationForm.append('max_participants_quantity', this.event.max_participants_quantity);
        DataCreationForm.append('tag', this.tags);


        axios.post(
          `http://eventrunner.1gb.ru/events/`,
          DataCreationForm,
      ...
      GetProfile:function( userid){
        ...
        if(this.stupid_counter==false) {
          axios.get('http://eventrunner.1gb.ru/users/' + userid)
            .then(response => (this.eventprofile = response["data"]));

          this.stupid_counter=true;
      ...
      nextTicket: function(event) {
        document.querySelector('.overlay').style.display = 'flex';
        document.querySelector('.ticket').style.display = 'none';
        document.querySelector('.ticket-success').style.display = 'flex';
      },
      closeNextTicket: function(event) {
        document.querySelector('.overlay').style.display = 'none';
        document.querySelector('.ticket-success').style.display = 'none';
      },
      ...
      likeEvent: function(event) {
        document.querySelector('.like-btn').classList.toggle('is-info');
        document.querySelector('.like-btn').classList.toggle('is-danger');
      },
    ...
    async created() {
      if (!store.getters.isLoggedIn) {
        this.EventDetailNoAuth=true;
        this.EventDetailAuth=false;
        this.NavbarAuth=false;
        this.NavbarNoAuth=true;
        this.$buefy.dialog.alert({
          title: 'Ошибка',
          message: 'Зарегестрируйтесь чтобы посмотреть информацию о мероприятии',
        ...
        this.EventDetailNoAuth=false;
        this.EventDetailAuth=true;
    ...
    mounted() {
      ...
      axios
        .get('http://eventrunner.1gb.ru/events/' + this.event_id)
        .then(response => (this.event_edit = response["data"]));
