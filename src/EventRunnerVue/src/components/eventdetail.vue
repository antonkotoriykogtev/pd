<template>
    ...
    <div class="tile is-parent">
      <article class="tile is-child">
        ...
        <div v-show="EventDetailAuth">
        <figure class="image ">
        <img
          :src="event_detail.picture_link"
          style="border-radius:10px;"
        >
        </figure>

        <h1 class="title is-1 MarginTextDetail" style="margin-bottom: 10px; display: inline-block;">{{ event_detail.name }}</h1>
          <b-button size="is-medium"
                    icon-left="heart"
                    v-on:click="PostLike(profile.id,event_detail.id)"
                    type="is-danger" outlined style="margin-top:1.5rem; "
          >
          </b-button>
          <div v-show="EventRegOff">

        <p class="template_info_type" style="margin-bottom: 10px;" v-if="event_detail.is_online">
          <b-icon icon="signal"></b-icon>
          Онлайн мероприятие</p>
        <p class="template_info_type" style="margin-bottom: 10px;"  v-if="!event_detail.is_online">
          <b-icon icon="building"></b-icon>
          Оффлайн мероприятие</p>
        <p class="template_info_date">{{ new Date(event_detail.start_date).toLocaleString('ru', {
          year: 'numeric',
          month: 'long',
          day: 'numeric',
          hour: 'numeric',
          minute: 'numeric',
          }) }} - {{ new Date(event_detail.end_date).toLocaleString('ru', {
          year: 'numeric',
          month: 'long',
          day: 'numeric',
          hour: 'numeric',
          ...
          <div v-if="!event_detail.is_online">
        <p class="title is-3" > Адрес:</p>

        <p class="subtitle is-5">{{ event_detail.address }}</p>
          </div>
        <div class="tile is-ancestor" style="margin-top:0.8rem">
              ...
              <div class="tile is-8 is-parent">
                <article class="tile is-child ">
                  <div class="box">
                    ...
                    <article :set="eventprofile = GetProfile(event_detail.author)" class="media">
                      <div class="media-left">
                        <figure class="image is-64x64  " >
                          <img class="is-rounded"  :src="eventprofile.picture_link" alt="Image">
                        </figure>
                      </div>
                      <div class="media-content">
                        <div class="content"  >
          ...
          <div class="buttons">
            <b-button type="is-success" size="is-medium" expanded v-on:click="EventReg">Регистрация на мероприятие</b-button>
          </div>

        <div class="template_content_description" v-html="event_detail.description"></div>
          </div>
          <div v-show="EventRegOn">
            <h1 class="title MarginTextDetail has-text-centered" style="margin-bottom: 10px;">Регистрация на мероприятие</h1>
            <b-steps
              type="is-info"
              v-model="activeStep"
              :animated="isAnimated"
              ...
              <b-step-item step="1" label="Регистрация" :clickable="isStepsClickable">
                <h1 class="title has-text-centered">Регистрация</h1>
                <b-field label="Выберите дату посещения" :label-position="labelPosition">
                  <b-datepicker
                    v-model="selected"
                ...
                <h3 class=" MarginTextDetail " style="margin-bottom: 10px;">Выберите вид билета</h3>
                  <div class="block">
                    ...
                    <b-radio v-model="radio"
                             name="name"
                             type="is-info"
                             native-value="Exemption">
                      Льготный билет для инвалидов, многодетных семей, пенсионеров, школьников и детей от 7 лет (будни и выходные)
                    </b-radio>
              ...
              <b-step-item step="2" label="Оплата" :clickable="isStepsClickable" >
                <h1 class="title has-text-centered">Оплата</h1>
             ...
             <!-- <b-step-item step="3" :visible="showSocial" label="Social" :clickable="isStepsClickable">
              ...
              <b-step-item :step="showSocial ? '4' : '3'" label="Расскажите друзьям!" :clickable="isStepsClickable" disabled>
                <h1 class="title has-text-centered">Расскажите друзьям!</h1>
            ...
            <div class="buttons">
              <b-button
                        type="is-light"
                        size="is-medium" expanded v-on:click="EventRegClose" style="margin-top:2rem">Отмена</b-button>
            </div>
          </div>
        </div>
      </article>
...
<script >
  ...
  methods: {
    EventReg:function(){
      this.EventRegOn=true;
      this.EventRegOff=false;
    },
    EventRegClose:function(){
      this.EventRegOn=false;
      this.EventRegOff=true;
    },
    PostLike:function(user_id, event_id){
      axios.post(
        `http://eventrunner.1gb.ru/user_in_event/`,
        {
          "user_id":user_id,
          "event_id":event_id
        },
  ...
  mounted() {
    axios
      ...
      .then(response => (this.event_detail = response["data"]));
