<template>
  ...
  <div class="row">
        ...
        <div class="column is-three-fifths">
          <div class="field is-grouped" >
            ...
            <p class="control is-expanded has-icons-left">
              <input class="input is-info is-rounded" v-model="EventName" v-on:keyup.enter="onEnterClick" @change="DeleteButton" type="text"
            ...
            <p class="control">
              <button class="delete is-large " v-show="SearchDeleteButton" v-on:click="DeleteButtonSearch" style="margin-top:4px; margin-left:10px;   "></button>
            </p>
          </div>

        </div>
        <div class="column" align="right">
          <b-button type="is-info" v-on:keyup.enter="LecturesSearch">Лекции</b-button>
          <b-button type="is-info" v-on:keyup.enter="MeetupSearch">Митапы</b-button>
          <b-button class=" button is-info" v-on:keyup.enter="CoursesSearch">Курсы</b-button>
        </div>
      </div>
    </div>
    <div class="container is-max-desktop container_padding" >
      ...
      <div class="columns">
        <div class="column">
          <h1 style="display: inline;"class="title is-3" >Мероприятия
          </h1>

          <b-dropdown :triggers="['hover']" v-model="isPublic"  aria-role="list"  style="font-family: 'Montserrat', sans-serif;">
            ...
            <b-dropdown-item  v-on:click="EventCity" value="moscow" aria-role="listitem">
                  <h3>Москвы</h3>
            </b-dropdown-item>
            <b-dropdown-item  v-on:click="EventCity" value="piter" aria-role="listitem">
                  <h3>Питера</h3>
            </b-dropdown-item>
          </b-dropdown>
        ...
        <div class="column " align="right">
          <button type="button" class="button is-success date-btn" name="button">Дата</button>
        </div>
      </div>
    </div>
    ...
    <div class="container is-max-desktop container_padding slider"   >


      <b-carousel
        :progress="progress"
        :progress-type="progressType">
        <b-carousel-item v-for="(data,index) in EventPromoted" :key="id">
          <router-link :to="'/eventdetail/' + data.id" class="link-item" >
          <section :class="`hero`">

            <div class="hero-body has-text-left" style="padding: 0rem 0rem;">

              <img :src="EventPromoted[0].picture_link" style="border-radius:10px">

              <h1 class="title" ><b-tag type="is-info is-light  title is-4" style="margin-top:-150px; margin-left:1rem;" >{{ EventPromoted[index].name }}</b-tag></h1>
            </div>
            <!--<div class="hero-body has-text-right" style="padding: 0rem 0rem;">

              <h1 class="title" ><b-tag type="is-info is-light  title is-5" style="margin-top:-200px; margin-right:1rem;" >{{ info[index].price }}</b-tag></h1>
            </div>-->
    ...
    <div class="container is-max-desktop EventContainerMargin" >
      ...
      <ul id='content_list' >
        <li v-for="(data,index) in EventSearch" :key="id">
          <router-link :to="'/eventdetail/' + data.id" class="link-item">
              ...
              <div class="card-image">
                <div class="item_logo">
                  <figure class="image is-16by9">
                    <img :src="EventSearch[index].picture_link" style=" border-radius: 8px 8px 0 0; object-fit: cover;" alt=' '>
                  </figure>
                  <div class="item_price" v-if="EventSearch[index].price === null">
                    <b-tag type="is-info">Бесплатно</b-tag>

                  </div>
                  <div class="item_price" v-else-if="EventSearch[index].price === ''">
                    <b-tag type="is-info">Бесплатно</b-tag>

                  </div>
                  <div class="item_price" v-else-if="EventSearch[index].price === 0">
                    <b-tag type="is-info">Бесплатно</b-tag>

                  </div>
                  <div class="item_price" v-else>
                    <b-tag type="is-info"> {{EventSearch[index].price}} р.</b-tag>
              ...
              <div class="card-content">
                <div class="media">
                  ...
                  <div class="media-content">
                    <p class="title is-4" style="margin-top:10px;">{{ EventSearch[index].name }}</p>
                    <div  v-if="EventSearch[index].end_date === null">
                      <p class="subtitle is-6">
                        {{ new Date(EventSearch[index].start_date).toLocaleString('ru', {
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric',
                        hour: 'numeric',
                    ...
                    <div v-else>
                      <p class="subtitle is-6">
                        ...
                        }) }} - {{ new Date(EventSearch[index].end_date).toLocaleString('ru', {
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric',
                        hour: 'numeric',
                ...
                <div class="content">
                  <b-field>
                    <b-taglist>
                      ...
                      <b-tag v-for="(data,TagsIndex) in EventSearch[index].tag" :key="id" type="is-info">{{EventSearch[index].tag[TagsIndex].name }} </b-tag>
    ...
    <div class="overlay">
      <div class="login">
        ...
        <h1>Вход</h1>
          ...
          <form class="checkin_form" @submit.prevent="login">
            <input type="email"  value="" maxlength="50"  placeholder="Почта" v-model="email">
            <input type="password"  value="" password-reveal  placeholder="Пароль" v-model="password">
            <button class='form-btn button is-info overlay-btn LoginButton' native-type="submit">Войти</button>
          </form>
      </div>
      ...
      <div class="checkin">
        ...
        <form class="checkin_form" v-on:submit.prevent="RegistrationForm">
          <input type="email"   placeholder="Почта" v-model="form.email">
  ...
  export default {
    ...
    methods: {
      EventCity(){
          ...
          {
            axios
              .get('http://eventrunner.1gb.ru/events-search/?city=' + "москва")
              .then(response => (this.EventSearch = response["data"]))
              .catch(e => console.log(e));
          }
          else
          {
            axios
              .get('http://eventrunner.1gb.ru/events-search/?city=' + "питер")
              .then(response => (this.EventSearch = response["data"]))
              .catch(e => console.log(e));
          }
      ...
      DeleteButton(){
        this.SearchDeleteButton=true;
      },
      DeleteButtonSearch(){
        document.querySelector('.slider').style.display = 'flex';
        this.SearchDeleteButton=false;
        axios
      ...
      onEnterClick() {
        ...
        axios
          .get('http://eventrunner.1gb.ru/events-search/?search=' + this.EventName)
          .then(response => (this.EventSearch = response["data"]))
          .catch(e => console.log(e));
      },
      RegistrationForm(){
      ...
      login: function () {
        ...
        axios({url: 'http://eventrunner.1gb.ru/api/token/', data: {email,password}, method: 'POST' })
          .then((response) => {
        ...
        this.$store.dispatch('login', { email, password })
          .then(localStorage.setItem('email', email))
           .then(() => this.$router.go())
          .then(this.$buefy.dialog.alert('Авторизация прошла успешно!'))
          .then(() => this.$router.go("/user"))
    ...
    async created() {
      axios
        .get('http://eventrunner.1gb.ru/events/')
        .then(response => (this.EventPromoted = response["data"]));
      axios
        .get('http://eventrunner.1gb.ru/events/')
        .then(response => (this.EventSearch = response["data"]));
